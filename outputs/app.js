...
function buildReports(workDate) {
  ...
  const currentRange = currentMonthRange(workDate);
  const current = reportableRows.flatMap((row) =>
    getOccurrencesInRange(row, currentRange.start, currentRange.end).map((date) => ({ row, date }))
  ).sort((a, b) => a.date - b.date || a.row[columns.holder].localeCompare(b.row[columns.holder]));
  ...
  return { activeRows, monthly, weekly, current, currentRange, birthdays, monthlyRange, weeklyStart, weeklyEnd };
}
...
function render() {
  ...
  els.currentTitle.textContent = `Pólizas de ${new Intl.DateTimeFormat("es-MX", { month: "long", year: "numeric" }).format(reports.currentRange.start)} (${reports.current.length})`;
  els.currentRows.innerHTML = reports.current.length ? reports.current.map(({ row, date }) => rowHtml([
    formatDate(date),
    row[columns.holder],
    row[columns.policy],
    row[columns.planName],
    ...
    row[columns.paymentMethod],
    formatAmount(getPaymentPremium(row)),
  ], reportLabels)).join("") : emptyHtml("No hay pólizas con cobro en el mes en curso.", 8);
